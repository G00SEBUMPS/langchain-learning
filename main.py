...
_OLLAMA_CB = CircuitBreaker(
    failure_threshold=3, recovery_timeout=20.0, expected_exception=Exception
)
_LLM_CB = CircuitBreaker(
    failure_threshold=2, recovery_timeout=30.0, expected_exception=Exception
)
...
def main():
    ...
    if ollama_env:
        ...
        if detected:
            ...
            print(
                f"Detected local Ollama at {ollama_base}; set OLLAMA_URL environment variable."
            )
        else:
            ollama_base = None
            print(
                "No local Ollama detected (tried http://localhost:11434). If you run Ollama locally, set OLLAMA_URL env var to point at it."
            )
    information = """
